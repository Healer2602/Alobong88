...
(function () {
    ...
    var readHeaders = function (editor) {
        ...
        return global$3.map(headers, function (h, i) {
            return {
                id: string_to_slug(editor.$.text(h)),
    ...
    var string_to_slug = function (text) {
    ...
    var generateTocContentHtml = function (editor) {
        ...
        html += generateTitle(Settings.getTocHeader(editor), global$2.translate('Nội dung'));
        for (i = 0; i < headers.length; i++) {
            h = headers[i];
            h.element.id = h.id;
