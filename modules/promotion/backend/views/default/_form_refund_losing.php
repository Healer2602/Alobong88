...
<div class="promotion-form">
	...
	<?= $form->beginField($model, 'refund_amount',
		['options' => ['class' => 'form-group']]) ?>
	<?= Html::activeLabel($model, 'refund_amount', ['class' => 'form-label']) ?>
	<div class="input-group input-group-merge">
		<?= Html::activeTextInput($model, 'refund_amount',
			['class' => 'form-control', 'type' => 'number', 'min' => 1]) ?>
		<div class="input-group-text rounded-end">
			<span class="fe fe-dollar-sign"></span>
		</div>
		<?= Html::error($model, 'refund_amount', ['class' => 'invalid-feedback']) ?>
	</div>
	<?= $form->endField() ?>
