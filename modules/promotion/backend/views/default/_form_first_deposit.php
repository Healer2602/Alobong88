...
<div class="promotion-form">
	...
	<?= $form->field($model, 'min_round')
	         ->textInput(['type' => 'number', 'min' => 0]) ?>

	<?= $form->beginField($model, 'min_deposit',
		['options' => ['class' => 'form-group']]) ?>
	<?= Html::activeLabel($model, 'min_deposit', ['class' => 'form-label']) ?>
	<div class="input-group input-group-merge">
		<?= Html::activeTextInput($model, 'min_deposit',
			['class' => 'form-control', 'type' => 'number', 'min' => 0]) ?>
		<div class="input-group-text rounded-end">
			<span class="fe fe-dollar-sign"></span>
		</div>
		<?= Html::error($model, 'min_deposit', ['class' => 'invalid-feedback']) ?>
	</div>
	<?= $form->endField() ?>

	<?= $form->beginField($model, 'max_bonus',
		['options' => ['class' => 'form-group']]) ?>
	<?= Html::activeLabel($model, 'max_bonus', ['class' => 'form-label']) ?>
	<div class="input-group input-group-merge">
		<?= Html::activeTextInput($model, 'max_bonus',
			['class' => 'form-control', 'type' => 'number', 'min' => 0]) ?>
		<div class="input-group-text rounded-end">
			<span class="fe fe-dollar-sign"></span>
		</div>
		<?= Html::error($model, 'max_bonus', ['class' => 'invalid-feedback']) ?>
	</div>
	<?= $form->endField() ?>

	<?= $form->field($model, 'number_promotion')
	         ->textInput(['type' => 'number', 'min' => 0]) ?>
	...
	<?= $form->beginField($model, 'maximum_promotion',
		['options' => ['class' => 'form-group']]) ?>
	<?= Html::activeLabel($model, 'maximum_promotion', ['class' => 'form-label']) ?>
	<div class="input-group input-group-merge">
		<?= Html::activeTextInput($model, 'maximum_promotion',
			['class' => 'form-control', 'type' => 'number', 'min' => 1]) ?>
		<div class="input-group-text rounded-end">
			<?= Yii::t('promotion', 'Day') ?>
		</div>
		<?= Html::error($model, 'maximum_promotion', ['class' => 'invalid-feedback']) ?>
	</div>
	<?= $form->endField() ?>
