...
namespace modules\media\libraries;
	...
	public function onUploadEnd($res){
		...
		if ($is_img){
			if (isset($this->options['config']['image_watermark']) && $this->options['config']['image_watermark']){
				...
				$magicianObj->addWatermark($this->options['config']['image_watermark'],
			...
			if ($thumbResult !== TRUE){
				if ($thumbResult === FALSE){
					$res['files'][0]->error = Yii::t('media', "Not enough Memory");
				}else{
					$res['files'][0]->error = $thumbResult;
				}
			}else{
				if (!$this->options['ftp'] && !new_thumbnails_creation($targetPath, $targetFile,
						$_FILES['files']['name'][0], $this->options['config']['thumbs_upload_dir'],
						$this->options['config'])){
					$res['files'][0]->error = Yii::t('media', "Not enough Memory");
				}else{
					...
					if ($this->options['config']['image_resizing']){
						...
						create_img($targetFile, $targetFile,
		...
		if ($this->options['ftp']){
			...
			$this->options['ftp']->put($targetPath . $res['files'][0]->name, $targetFile,
				FTP_BINARY);
			unlink($targetFile);
			if ($is_img){
				$this->options['ftp']->put($targetPathThumb . $res['files'][0]->name,
					$targetFileThumb, FTP_BINARY);
