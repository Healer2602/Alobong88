...
<?php $this->beginPage() ?>
	...
	<head>
		...
		<?php if (!empty($this->setting->gtm)): ?>
			...
			<script>(function (w, d, s, l, i) {
				...
				})(window, document, 'script', 'dataLayer', '<?= $this->setting->gtm?>');</script>            <!-- End Google Tag Manager -->
		<?php endif; ?>
	</head>
	...
	<?php if (!empty($this->setting->gtm)): ?>
		...
		<noscript>
			<iframe src="https://www.googletagmanager.com/ns.html?id=<?= $this->setting->gtm ?>" height="0" width="0" style="display:none;visibility:hidden"></iframe>
			...
			<nav class="navbar navbar-expand-lg navbar-light">
				...
				if ($languages && count($languages) > 1):
					...
					<div class="dropdown language-switcher">
						<a class="dropdown-toggle language <?= Language::current() ?>" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
							<span><?= Html::encode(Language::currentName()) ?></span>
						</a>
						<ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdown-menu">
							<?php foreach ($languages as $code => $language): ?>
								<li>
									<a class="dropdown-item language <?= $code ?>" href="<?= Url::current(['lang' => $code]) ?>">
