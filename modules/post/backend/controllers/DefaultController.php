...
class DefaultController extends Controller{
	...
	public function actionIndex(){
		if ($this->request->isPost && Yii::$app->user->can('product delete')){
		...
		$query = Post::find()->with('category', 'author');
		...
		$query->andFilterWhere(['category_id' => $filtering['category'] ?? NULL]);
	...
	public function actionCreate(){
		...
		if ($model->load(Yii::$app->request->post()) && $model->save()){
			$this->flash('success', Yii::t('post', 'Post has been created successfully'));
	...
	public function actionUpdate($id){
		...
		if ($model->load(Yii::$app->request->post()) && $model->save()){
			$this->flash('success', Yii::t('post', 'Post has been updated successfully'));
	...
	protected function batchDelete(){
		...
		if (!empty($post_ids)){
			$total_deleted = Post::deleteAll(['id' => $post_ids]);

			if ($total_deleted){
				$this->flash('success', Yii::t('post', 'There were {0} post(s) deleted.'),
					[$total_deleted]);
			}else{
				$this->flash('error',
					Yii::t('post', 'There were no posts deleted. Please try again.'),
					[$total_deleted]);
			}
		}else{
			$this->flash('warning', Yii::t('post', 'Please select some posts before delete.'));
		}
